...
class sale_order_list(osv.Model):
	...
	_columns = {
		...
		'total' : fields.function(_get_total, type='float', string="Total"),
	...
	def action_order_list_send(self, cr, uid, ids, context=None):
		...
		try:
			template_id = ir_model_data.get_object_reference(cr, uid, 'sale_order_list', 'email_template_edi_sale_order_list')[1]
		except ValueError:
			template_id = False
		try:
