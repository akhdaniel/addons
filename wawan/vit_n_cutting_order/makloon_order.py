			...
			for ls in loop_size:
				if obj_mrp.size == ls:
					...
					data_line = {
							...
							'location_dest_id'  : lokasi_scrapped_id,
							}
					...
					data_line_b = {
							...
							'location_id'       : lokasi_scrapped_id,
...
class vit_makloon_order(osv.osv):
	...
	def _get_jasa_makloon(self, cr, uid, context=None):
		...
		if product ==[] :
			raise osv.except_osv( 'Tidak Ditemukan!' , 'Product : Name :Jasa Maklon, Categ_id : Jasa, Expense Account: 5-311002 Upah Jahit')
		else:
	...
	def action_inprogress(self, cr, uid, ids, context=None):
		...
		self.pool.get('account.invoice.line').create(cr,uid,{'invoice_id' : inv_makloon,
																...
																'name':prod_name,
																'account_id': self.pool.get('product.product').browse(cr,uid,1,context=context).categ_id.property_account_expense_categ.id,
