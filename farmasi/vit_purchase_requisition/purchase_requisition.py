...
class purchase_requisition_line(osv.osv):
    ...
    _columns = {
        ...
        'requisition_date': fields.related('requisition_id','schedule_date',
            type="text",
            relation="purchase_requisition.purchase.requisition",
    ...
    def action_createPO(self, cr, uid, context=None):
        ...
        cr.commit()
        raise osv.except_osv( 'OK!' , 'Done creating %s RFQ(s) ' % (i) )        
...
class purchase_requisition(osv.osv):
    ...
    def _planned_date(self, requisition, delay=0.0):
        ...
        if requisition.ordering_date:
            date_planned = datetime.strptime(requisition.ordering_date, '%Y-%m-%d %H:%M:%S') - relativedelta(days=company.po_lead)
        else:
