...
class stock_picking_in(osv.osv):
	...
	def action_process(self, cr, uid, ids, context=None):
		res = super(stock_picking_in, self).action_process(cr, uid, ids, context=None)
		# import pdb;pdb.set_trace()
		self.browse(cr,uid,ids[0],context).reward_steam
		return res
	...
	def create_invoice(self, cr, uid, ids, context=None):
		...
		# spk_id = self.pool.get('vit.makloon.order').search(cr,uid,[('name','=',origin)],context=context)[0]
...
class stock_partial_picking(osv.osv_memory):
	...
	def do_partial(self, cr, uid, ids, context=None):
		...
		origin_makloon = partial.move_ids[0].move_id.picking_id.origin 
		res = super(stock_partial_picking, self).do_partial(cr, uid, ids, context=context)


		# import pdb;pdb.set_trace()
		spk_id= self.pool.get('vit.makloon.order').search(cr,uid,[('name','=',origin_makloon)],context=context)
		if spk_id == []:
			return
		self.pool.get('vit.makloon.order').write(cr,uid,spk_id[0],{'date_received':datetime.date.today()})
		return res
