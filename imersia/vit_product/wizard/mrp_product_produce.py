...
class mrp_product_produce_custom(osv.osv_memory):
    _name = "mrp.product.produce.custom"
    _description = "Product Produce"

    _columns = {
        'product_id': fields.many2one('product.product', 'Product Name',type='many2one'),
      ...
      #  'consume_lines': fields.one2many('mrp.production.custom.product.line', 'produce_id', 'Products Consumed'),
    ...
    def on_change_qty(self, cr, uid, ids, product_qty, consume_lines, context=None):
        ...
        """
        prod_obj = self.pool.get("mrp.production.custom")
    ...
    def _get_product_qty(self, cr, uid, context=None):
        ...
        if context is None:
            context = {}
        prod = self.pool.get('mrp.production.custom').browse(cr, uid,
    ...
    def do_produce(self, cr, uid, ids, context=None):
        ...
        self.pool.get('mrp.production.custom').action_produce(cr, uid, production_id,
                            data.product_qty, data.mode, data, context=context)
        return {}
    
"""
class mrp_production_custom_product_line(osv.osv_memory):
    ...
    _columns = {
      ...
      #  'produce_id': fields.many2one('mrp.product.produce.custom'),
